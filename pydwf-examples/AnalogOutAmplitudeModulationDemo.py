...
"""This demo shows the effect of amplitude modulation in the AnalogOut instrument.
...
from pydwf import (DwfLibrary, DwfEnumConfigInfo, DwfAnalogOutNode, DwfAnalogOutFunction, PyDwfError, DwfTriggerSource,
                   DwfAnalogInFilter, DwfAcquisitionMode, DwfState, DwfAnalogOutIdle)
from pydwf.utilities import openDwfDevice


def analog_output_am_modulation_demo(device, num_periods):
    ...
    while True:
        ...
        plt.suptitle("Effect of the symmetry setting on different AM wave-shape functions\nsymmetry value = {}".
                     format(symmetry_setpoint))

        for (waveform_index, waveform_func) in enumerate(waveform_functions, 1):

            # Prepare the analog-out device

            analogOut.reset(-1)

            # Set carrier signal

            set_analog_output_node_settings(
                analogOut, CH1, DwfAnalogOutNode.Carrier,
                enable    = True,
                func      = DwfAnalogOutFunction.Sine,
                frequency = input_frequency_setpoint / 16384 * num_periods * 10,
                amplitude = 2.0, # [V]
                offset    = 0.0, # [V]
                symmetry  = 50.0,
                phase     = 0.0)
            ...
            set_analog_output_node_settings(
                analogOut, CH1, DwfAnalogOutNode.AM,
                enable    = True,
                func      = waveform_func,
            ...
            am_settings      = get_analog_output_node_settings(analogOut, CH1, DwfAnalogOutNode.AM)
            ...
            predicted_samples = analog_output_signal_simulator(carrier_settings, am_settings, None, t)
        ...
        symmetry_setpoint = symmetry_setpoint  + 5
        if symmetry_setpoint > 100:
            symmetry_setpoint = 0
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Demonstrate effect of the AnalogOut symmetry setting.")
